...
async function requireOwner(ctx: QueryCtx | MutationCtx, eventId: Id<"events">) {
...
export const syncProfile = mutation({
  ...
  handler: async (ctx) => {
    ...
    const profile = {
      ...
      email: identity.emailVerified ? identity.email?.trim().toLowerCase() : undefined,
      phoneNumber: identity.phoneNumberVerified ? identity.phoneNumber?.trim() : undefined,
      updatedAt: Date.now(),
    };
...
export const claim = mutation({
  ...
  handler: async (ctx) => {
    ...
    if (!identity.emailVerified || identity.email === undefined) {
      throw new Error("A verified email address is required to accept invitations");
    }
    const email = normalizedEmail(identity.email);
    const invitations = await ctx.db
      .query("eventInvitations")
      .withIndex("by_email_status", (index) => index.eq("email", email).eq("status", "pending"))
      .collect();
    for (const invitation of invitations) {
      const membership = await ctx.db
        .query("eventMemberships")
        .withIndex("by_eventId_userId", (index) => index.eq("eventId", invitation.eventId).eq("userId", identity.subject))
        .unique();
      if (membership === null) {
...
export const create = mutation({
  ...
  handler: async (ctx, { eventId, email, role }) => {
    ...
    const existing = await ctx.db.query("eventInvitations").withIndex("by_eventId_email", (index) => index.eq("eventId", eventId).eq("email", normalized)).unique();
    if (existing !== null && existing.status === "pending") throw new Error("This email already has a pending invitation");
    const values = {
      eventId,
      email: normalized,
      role,
...
export const revoke = mutation({
  ...
  handler: async (ctx, { eventId, invitationId }) => {
    ...
    if (invitation === null || invitation.eventId !== eventId) throw new Error("Invitation not found");
    if (invitation.status === "pending") await ctx.db.patch(invitationId, { status: "revoked" });
  },
});

export const updateMemberRole = mutation({
  args: { eventId: v.id("events"), membershipId: v.id("eventMemberships"), role: invitationRole },
  handler: async (ctx, { eventId, membershipId, role }) => {
    await requireOwner(ctx, eventId);
    const membership = await ctx.db.get(membershipId);
    if (membership === null || membership.eventId !== eventId) throw new Error("Member not found");
    if (membership.role === "owner") throw new Error("An event owner cannot be demoted");
...
export const removeMember = mutation({
  ...
  handler: async (ctx, { eventId, membershipId }) => {
    ...
    const membership = await ctx.db.get(membershipId);
    if (membership === null || membership.eventId !== eventId) throw new Error("Member not found");
    if (membership.role === "owner") throw new Error("An event owner cannot be removed");
...
export const listContacts = query({
  ...
  handler: async (ctx, { eventId }) => {
    await requireOwner(ctx, eventId);
    const memberships = await ctx.db.query("eventMemberships").withIndex("by_eventId_userId", (index) => index.eq("eventId", eventId)).collect();
    const people = await Promise.all(memberships.map(async (membership) => {
      const profile = await ctx.db.query("userProfiles").withIndex("by_userId", (index) => index.eq("userId", membership.userId)).unique();
      return { id: membership._id, type: "member" as const, role: membership.role, userId: membership.userId, name: profile?.displayName, email: profile?.email, phoneNumber: profile?.phoneNumber };
    }));
    const invitations = await ctx.db.query("eventInvitations").withIndex("by_eventId", (index) => index.eq("eventId", eventId)).collect();
    return [...people, ...invitations.filter((invitation) => invitation.status === "pending").map((invitation) => ({ id: invitation._id, type: "invitation" as const, role: invitation.role, email: invitation.email }))];
  },
});
